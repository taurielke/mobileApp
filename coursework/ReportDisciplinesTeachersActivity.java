...
import com.example.coursework.deanery.ReportsLogic.DisciplinesTeachers;
...
public class ReportDisciplinesTeachersActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_report_disciplines_teachers);
        BaseFont bf= null;
        try {
        ...
        List<DisciplinesTeachers> list = reportsLogic.getDisciplinesByTeachers(login);

        TableLayout table = findViewById(R.id.reportDisciplineTeacherTable);
        TableRow head = new TableRow(this);
        head.setLayoutParams(new TableRow.LayoutParams(
                TableRow.LayoutParams.MATCH_PARENT,
                TableRow.LayoutParams.WRAP_CONTENT));
        TextView h1 = new TextView(this);
        h1.setText("Преподаватель");
        h1.setTextSize(18);
        ...
        Button save = findViewById(R.id.reportDisciplineTeacherToPdf);
        save.setOnClickListener(v -> {
            try {
                if (!checkPermission()) {
                    requestPermission();
                }
                if (!ReportDisciplinesTeachersActivity.PermissionUtils.hasPermissions(this)) {
                    ReportDisciplinesTeachersActivity.PermissionUtils.requestPermissions(this, PERMISSION_STORAGE);
                }
                String filename="Преподаватели.pdf";
                ...
                document.open();
                Paragraph p3=new Paragraph("Преподаватели и их дисциплины", font);
                document.add(p3);
                ...
                tablePdf.addCell(new Phrase(Element.ALIGN_LEFT,"Преподаватель",font));
                tablePdf.addCell(new Phrase(Element.ALIGN_LEFT,"Дисциплины",font));
                tablePdf.setWidths(new int[]{150,120});

                for(DisciplinesTeachers trGui : list){
                    tablePdf.addCell(new Phrase(Element.ALIGN_LEFT,trGui.getTeacher(),font));
                    tablePdf.addCell(new Phrase(Element.ALIGN_LEFT,trGui.getDiscipline(),font));
                }
